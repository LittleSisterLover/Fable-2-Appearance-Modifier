...
My primary goal was to remove will lines from my character; I want to use magic, but not at the cost of being ugly. I've added the other functions and released this mod for those with similar issues but not necessarily the skillset to address it. I'm not entirely satisfied with the implementation - in large part due to the inability to edit skin color without touching Morality/Purity values - but did not see another way to make it work within the current restrictions. At the moment, the most promising lead for a better system would be within the game's Global Database, which appears to hold morphing values, but this has not been deconstructed in any way and requires a lot more work to understand. If I find something to that end or encounter a serious issue with the current system, I may update the mod to address it.
